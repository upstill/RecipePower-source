(function() {
  var collection_beforeSend, collection_onload, collection_success, collection_tagchange, collection_update_success;
  ...
  jQuery(function() {
    ...
    $(window).resize(function() {
      ...
      if ((elmt = $("div.browser_house")[0]) && (navlinks = $('div#footer_nav_links')[0])) {
        return elmt.style.bottom = (navlinks.offsetHeight + 5).toString() + "px";
      }
    });
    if ((elmt = $("div.browser_house")[0]) && (navlinks = $('div#footer_nav_links')[0])) {
    ...
    $('a.update-button').click(function() {
      ...
      if (url = $(this).data("url")) {
        return RP.get_content(url, 'a.update-button');
      }
    });
    collection_onload();
    return RP.fire_triggers();
  });

  collection_onload = function() {
    $("#tagstxt").first().focus();
    ...
    return window.setTimeout(function() {
      return RP.collection.justify();
    }, 800);
  };

  collection_tagchange = function() {
    var formitem;
    formitem = $('form.query_form');
    if ($(formitem).data("format") === "html") {
      return formitem.submit();
    } else {
      return RP.collection.update($(formitem).serialize(), $(formitem).attr("action"));
    }
  };

  collection_success = function(evt, data, status, xhr) {
    return collection_update_success(data);
  };

  collection_beforeSend = function(evt, xhr, settings) {
    RP.dialog.cancel();
    RP.notifications.wait();
    return true;
  };

  collection_update_success = function(resp) {
  ...
  RP.collection.update = function(params, url) {
    ...
    return jQuery.ajax({
      ...
      error: function(jqXHR, textStatus, errorThrown) {
        ...
        return RP.process_response(responseData);
      },
      success: function(resp, succ, xhr) {
        return collection_update_success(resp);
      }
    });
  };

  RP.collection.rejustify = function() {
    return $('#masonry-container').masonry();
  };

  RP.collection.justify = function() {
    return $('#masonry-container').masonry({
      columnWidth: 220,
      gutter: 20,
...
(function() {
  ...
  RP.authentication || (RP.authentication = {});

  popupCenter = function(url, width, height, name, msg) {
    var left, myWindow, top;
    ...
    myWindow.focus();
    return myWindow.location = url;
  };

  RP.authentication.connect = function(e) {
    var link;
...
(function() {
  var cancel_modal, filter_submit, manager_of, notify, notify_injector, open_modal, target_modal;
  ...
  jQuery(function() {
    ...
    return $(document).on("ajax:error", '.dialog-run', RP.dialog.error);
  });

  RP.dialog.arm_links = function(dlog) {
    var fcn, requirement, requires, _i, _len, _results;
    dlog || (dlog = window.document);
    ...
    if (requires = $(dlog).data('dialog-requires')) {
      ...
      for (_i = 0, _len = requires.length; _i < _len; _i++) {
        requirement = requires[_i];
        if (fcn = RP.named_function("RP." + requirement + ".bind")) {
          _results.push(fcn.apply());
        } else {
  ...
  RP.dialog.beforeSend = function(event, xhr, settings) {
    var ndlog, odlog, selector;
  ...
  RP.dialog.success = function(event, responseData, status, xhr) {
    responseData.how = responseData.how || "modal";
    RP.post_success(responseData);
    return RP.process_response(responseData, target_modal(event));
  };

  RP.dialog.error = function(event, jqXHR, status, error) {
    var odlog, responseData;
    responseData = RP.post_error(jqXHR);
    odlog = target_modal(event);
    return RP.process_response(responseData, odlog);
  };

  RP.dialog.get_and_go = function(event, request, selector) {
    if (RP.dialog.beforeSend(event)) {
      return $.ajax({
        ...
        url: request,
        error: function(jqXHR, textStatus, errorThrown) {
          return RP.dialog.error(event, jqXHR, textStatus, errorThrown);
        },
        success: function(responseData, statusText, xhr) {
          return RP.dialog.success(event, responseData, statusText, xhr);
        }
      });
    }
  };

  RP.dialog.cancel = function(event) {
    var dlog;
    if (event) {
    ...
    if (dlog) {
      return RP.dialog.close_modal(dlog);
    }
  };

  RP.dialog.run = function(dlog) {
    return open_modal(dlog, true);
  };

  RP.dialog.extract_modal = function(code) {
    var dlog, doc, newdlog, wrapper;
  ...
  RP.dialog.supplant_modal = function(dlog, code) {
    var newdlog;
  ...
  RP.dialog.replace_modal = function(newdlog, odlog) {
    if (odlog) {
  ...
  open_modal = function(dlog, omit_button) {
    var buttoncode, fcn, focus_elmt, focus_sel, onget;
  ...
  RP.dialog.close_modal = function(dlog, epilog) {
    if (dlog) {
      if (($(dlog).modal)) {
        $(dlog).modal('hide');
      }
  ...
  cancel_modal = function(dlog, check_for_triggers) {
    if (check_for_triggers == null) {
      check_for_triggers = true;
    }
    if (($(dlog).modal)) {
      $(dlog).modal('hide');
    }
  ...
  filter_submit = function(eventdata) {
    var clicked, confirm_msg, context, dlog, method, shortcircuit, wait_msg;
    context = this;
    dlog = eventdata.data;
    ...
    if (($(clicked).attr("value") === "Save") && (shortcircuit = notify("beforesave", dlog, eventdata.currentTarget))) {
      ...
      $(context).ajaxSubmit({
        ...
        error: function(jqXHR, textStatus, errorThrown) {
          var jsonout;
          RP.notifications.done();
          jsonout = RP.post_error(jqXHR, dlog);
          eventdata.preventDefault();
  ...
  manager_of = function(dlog) {
    var classList, classname, mgr_name, _i, _len;
    if (dlog) {
  ...
  RP.dialog.user_note = function(msg) {
    return jNotify(msg, {
  ...
  notify = function(what, dlog, entity) {
    var fcn, fcn_name, hooks, mgr, msg_name;
      ...
      case "onopen":
        ...
        $('div.btn-group[data-toggle-name=*]').each(function() {
          ...
          return $('button', group).each(function() {
            ...
            return button.live('click', function() {
  ...
  notify_injector = function(what, dlog) {
    var fcn;
  ...
  RP.dialog.onopen = function(dlog, entity) {
    return notify('open', dlog, entity);
  };

  RP.dialog.onclose = function(dlog, entity) {
    return notify('close', dlog, entity);
  };

  RP.dialog.onload = function(dlog, entity) {
    return notify('load', dlog, entity);
  };

  RP.dialog.onsave = function(dlog, entity) {
    return notify('save', dlog, entity);
  };
...
;
(function() {
  var do_request, get_content, poll_for_update;

  window.RP = window.RP || {};

  RP.servePopup = function() {
    var linkURL, popUp, rcpid, regexp;
  ...
  RP.makeExpandingArea = function(containers) {
    var area, container, i, span;
    i = 0;
    ...
    return containers.addClass('active');
  };
  ...
  */
  ...
  RP.named_function = function(str) {
    var i, obj, strs;
    if (str) {
      obj = window;
      strs = str.split('.');
  ...
  RP.fire_triggers = function() {
    var dlog, elmt;
    if (dlog = $('div.dialog.trigger')[0]) {
      return RP.dialog.run(dlog);
    } else if ((elmt = $("a.trigger")[0])) {
      $(elmt).trigger("click");
      debugger;
      return $(elmt).removeClass("trigger");
    }
  };

  RP.showhide = function(event) {
    var associated, show_sib;
    ...
    if (show_sib) {
      $(associated).show(300);
      return $(associated).removeClass("hide");
    }
  };

  do_request = function(url, ajax_options, processing_options) {
    var e, xmlhttp;
      ...
      xmlhttp.onreadystatechange = function() {
        if (xmlhttp.readyState === 4) {
          if (xmlhttp.status === 200) {
            return $(processing_options.contents_selector).inner_html = xmlhttp.responseText;
          } else {
            debugger;
          }
        }
  ...
  poll_for_update = function(url, ajax_options, processing_options) {
    ajax_options.success || (ajax_options.success = function(resp, succ, xhr) {
      if (xhr.status === 204) {
        return setTimeout((function() {
          return poll_for_update(url, ajax_options, processing_options);
        }), 1000);
      } else if (xhr.status === 200) {
        if (ajax_options.dataType === "html") {
          return $(processing_options.contents_selector).html(xhr.responseText);
        } else {
          debugger;
        }
      } else {
        return RP.notifications.post("Sorry: got an error trying to update", "flash-error");
      }
    });
    ajax_options.error || (ajax_options.error = function(jqXHR, textStatus, errorThrown) {
      var dom;
      dom = $(jqXHR.responseText);
      if (dom.length > 0) {
        RP.notifications.post("Update complete!", "flash-alert");
        return $(processing_options.contents_selector).replaceWith(dom);
      } else {
        return RP.notifications.post(jqXHR.responseText, "flash-alert");
      }
    });
    return jQuery.ajax(url, ajax_options);
  };

  get_content = function(url, last_modified, options) {
    var ajax_options, hold_msg, msg_selector, processing_options;
    ...
    ajax_options.requestHeaders || (ajax_options.requestHeaders = {});
  ...
  RP.get_content = function(url, link) {
    ...
    return get_content(url, last_modified, {
  ...
  RP.get_page = function(url) {
    return $('body').load(url, {}, function(responseText, textStatus, XMLHttpRequest) {
  ...
  RP.post_success = function(jsonResponse, dlog, entity) {
    var closer;
    if (closer = RP.named_function(jsonResponse.processorFcn)) {
      closer(jsonResponse);
    }
    if ((dlog !== void 0) || (entity !== void 0)) {
      RP.dialog.onsave(dlog, entity);
    }
    return false;
  };

  RP.post_error = function(jqXHR, dlog) {
    var baseErrorDiv, clicked, e, errors, errtxt, form, formnode, newdlog, node, parsage, result, wrapper, _i, _len, _ref;
    parsage = {};
  ...
  RP.detach = function(node) {
    var parent;
    parent = node.parentNode;
    return parent.removeChild(node);
  };

  RP.change = function(event) {
    var attrname, attrvalue, data, elmt, str, _ref;
    elmt = event.target;
    data = $(elmt).data();
    if (data.valueparam) {
    ...
    return RP.submit.submit_and_process(data.request + "?" + str.join("&"), "GET", data);
  };

  RP.process_response = function(responseData, dlog) {
    var action, code, deletion, deletions, form, newdlog, page, redirect, replaced, replacement, replacements, supplanted, _i, _j, _len, _len1;
    dlog || (dlog = $('div.modal')[0]);
    supplanted = false;
    if (responseData) {
      ...
      if ((code = responseData.code) && !(supplanted = RP.dialog.supplant_modal(dlog, code))) {
        responseData.page || (responseData.page = code);
      }
...
(function() {
  ...
  RP.submit.bind = function(dlog) {
    dlog || (dlog = window.document);
    $(dlog).on("ajax:beforeSend", '.talky-submit', RP.submit.beforeSend);
    $(dlog).on("ajax:success", '.talky-submit', RP.submit.success);
    return $(dlog).on("ajax:error", '.talky-submit', RP.submit.error);
  };

  RP.submit.beforeSend = function(event, xhr, settings) {
    var confirm_msg, wait_msg;
  ...
  RP.submit.success = function(event, responseData, status, xhr) {
    RP.notifications.done();
    responseData.how = responseData.how || "modal";
    RP.post_success(responseData);
    return RP.process_response(responseData, target_modal(event));
  ...
  RP.submit.error = function(event, jqXHR, status, error) {
    var odlog, responseData;
    RP.notifications.done();
    responseData = RP.post_error(jqXHR);
    odlog = target_modal(event);
  ...
  RP.submit.go = function(event, request) {
    var attribs, confirm_msg, data, elmt, method;
    ...
    if (confirm_msg = attribs.confirm && attribs.confirm.value) {
      bootbox.confirm(confirm_msg, function(result) {
        if (result) {
          return RP.submit.submit_and_process(request, method, data);
        }
      });
    } else {
  ...
  RP.submit.submit_and_process = function(request, method, assumptions) {
...
(function() {
  var bootbox_alert, insert_flash, jnotify_popup;

  RP.notifications || (RP.notifications = {});

  RP.notifications.post = function(msg, how) {
    var level;
    if (msg && msg.length > 0) {
      ...
      return ((how === "flash") && insert_flash(msg, level)) || ((how === "alert") && (bootbox_alert(msg))) || ((how === "popup") && (jnotify_popup(msg))) || insert_flash(msg, "alert") || bootbox_alert(msg) || jnotify_popup(msg);
    }
  };

  RP.notifications.wait = function(msg) {
    var elmt, parent;
    if (elmt = $('div.ajax-loader')[0]) {
      ...
      return $(elmt).addClass("loading");
    }
  };

  RP.notifications.done = function(msg) {
    $('div.ajax-loader').removeClass("loading");
    if (msg) {
      return RP.notifications.post(msg);
    }
  };

  RP.notifications.html = function(msg) {
    return "<span style=\"text-align:center\"><strong>" + msg + "</strong></span>";
  };

  RP.notifications.from_response = function(data) {
    ...
    return RP.notifications.post(data.popup, "popup");
  };

  jnotify_popup = function(msg) {
    var available;
  ...
  insert_flash = function(message, level) {
    var available, bootstrap_class, html;
    if (available = $('div.flash_notifications')[0]) {
      bootstrap_class = "alert-" + level;
      html = "<div class=\"alert " + bootstrap_class + "	alert_block fade in\">		      <button class=\"close\" data-dismiss=\"alert\">&#215;</button>" + message + "</div>";
      $('div.flash_notifications').replaceWith(html);
    }
    return available;
  };

  bootbox_alert = function(msg) {
    var available;
...
(function() {
  var elementLevel, isAncestor, parentOf, toggleChildren;

  RP.content_browser = RP.content_browser || {};

  RP.content_browser.onload = function() {
    $('.RcpBrowser').click(function() {
      var data, selected, toClose;
      if (!$(this).hasClass("active")) {
    ...
    return $('.delete_element').bind('ajax:error', RP.content_browser.failed_deletion);
  };

  RP.content_browser.click_to_browser = function(e) {
    var elem, elemHeight, elemPosition, elemPosition2, elemWidth, elements, i, inside, me;
    inside = null;
    me = e.currentTarget;
  ...
  isAncestor = function(ancestor, descendant) {
    ...
    return prior === ancestor;
  };

  elementLevel = function(elmt) {
    var cn, ix;
    cn = elmt.className;
    ix = cn.indexOf('Level');
    if (ix > 0) {
      return cn.charAt(ix + 5);
    } else {
      return "0";
    }
  };

  toggleChildren = function(me) {
    var myLevel, _results;
    myLevel = elementLevel(me);
    _results = [];
    while ((me = $(me).next()[0]) && (elementLevel(me) > myLevel)) {
      _results.push($(me).toggle(200));
    }
    return _results;
  };

  RP.content_browser.failed_deletion = function(evt, xhr, status, error) {
    return $('.notifications-panel').html(xhr.responseText);
  };

  RP.content_browser.div = RP.content_browser.div || $('<div></div>');

  RP.content_browser.insert_or_select = function(resp) {
    var div, elmt, entities, entity, existing, parent_elmt, parent_id, target, _i, _len;
  ...
  jQuery(function() {
    return RP.content_browser.onload();
  });

}).call(this);
(function() {
  RP.edit_feed || (RP.edit_feed = {});

  RP.edit_feed.onload = function() {
    return RP.tagger.onload("#feed_tag_tokens");
  };

}).call(this);
(function() {
  var me, recipedata, tagger_selector,
    __hasProp = {}.hasOwnProperty;

  RP.edit_recipe = RP.edit_recipe || {};

  me = function() {
    return $('div.edit_recipe');
  };

  tagger_selector = "div.edit_recipe #recipe_tag_tokens";

  RP.edit_recipe.go = function(evt, xhr, settings) {
    var dlgsource, dlog, rcpdata, templ, template;
    rcpdata = $(this).data();
    ...
    if (templ = $(template).data("template")) {
      dlgsource = templ.string.replace(/%%rcpID%%/g, rcpdata.rcpid).replace(/%%rcpTitle%%/g, rcpdata.rcptitle).replace(/%%rcpPicURL%%/g, rcpdata.rcppicurl || "/assets/NoPictureOnFile.png").replace(/%%rcpPrivate%%/g, rcpdata.rcpprivate).replace(/%%rcpComment%%/g, rcpdata.rcpcomment).replace(/%%rcpStatus%%/g, rcpdata.rcpstatus).replace(/%%authToken%%/g, rcpdata.authtoken);
      $(template).html(dlgsource);
    }
    RP.tagger.init(tagger_selector, rcpdata.rcptagdata);
    RP.dialog.run(me());
    return false;
  };

  RP.edit_recipe.onload = function(dlog) {
    var dataBefore, rcpid, touch_recipe;
    dlog = me();
    if ($('.edit_recipe > *').length > 0) {
      RP.tagger.onload(tagger_selector);
      if ($('.pic_picker_golink', dlog).length > 0) {
        RP.pic_picker.load(function(picdlg) {
          return $('.pic_picker_golink', dlog).removeClass('hide');
        });
        $(".pic_picker_golink", dlog).click(function() {
          event.preventDefault();
      ...
      if (touch_recipe = RP.named_function("RP.rcp_list.touch_recipe")) {
        return touch_recipe(rcpid);
      }
    }
  };

  RP.edit_recipe.onclose = function(dlog) {
    var picker_dlog;
  ...
  recipedata = function(arr) {
    ...
    $.each(arr, function() {
      ...
      if (this.name.match(/recipe\[.*\]$/)) {
        ...
        return result[index] = this.value;
      }
    });
    return result;
  };

  RP.edit_recipe.submission_redundant = function(dlog) {
    var attr, dataAfter, dataBefore, hooks, value;
    ...
    for (attr in dataAfter) {
      if (!__hasProp.call(dataAfter, attr)) continue;
      value = dataAfter[attr];
      if (dataBefore[attr] !== value) {
        return null;
      }
    }
    return {
      done: true,
      popup: "Sorted! Cookmark secure and unchanged."
    };
  };

}).call(this);
(function() {
  var add_expression, me, tagger_selector;

  RP.edit_referent = RP.edit_referent || {};

  me = function() {
    return $('div.edit_referent');
  };

  tagger_selector = "div.edit_referent input#referent_add_expression";

  add_expression = function(hi, li) {
    var newfields, other, regexp, tagid, that, time, valstr;
    that = $('.add_fields');
    ...
    return $('#referent_add_expression').tokenInput("remove", {
      id: hi.id
    });
  };

  RP.edit_referent.onload = function(dlog) {
    var querystr, tagtype;
    dlog = me();
    RP.tagger.onload(tagger_selector);
    ...
    return $("#referent_add_expression").tokenInput(querystr + "&untypedOK=1", {
      crossDomain: false,
      noResultsText: "No existing tag found; hit Enter to make a new tag",
      hintText: "Type/select another tag to express this thing",
      theme: "facebook",
      tokenLimit: 1,
      onAdd: add_expression,
      preventDuplicates: true,
      allowFreeTagging: true
    });
  };
...
(function() {
  ...
  me = function() {
    return $('div.edit_site.dialog');
  };

  RP.edit_site.onload = function(dlog) {
    dlog = me();
    if ($('.edit_site > *').length > 0) {
      ...
      return fitImageOnLoad("div.pic_preview img");
    }
  };
  ...
  jQuery(function() {
    var dlog;
    if (dlog = me()[0]) {
      return RP.edit_site.onload(dlog);
    }
  });

}).call(this);
(function() {
  RP.edit_user || (RP.edit_user = {});

  RP.edit_user.onload = function() {
    return RP.tagger.onload("#user_tag_tokens");
  };
...
(function() {
  ...
  RP.new_recipe.onclose = function(dlog) {
    var result;
    if (result = dialogResult(dlog)) {
      return RP.rcp_list.update(result);
    }
  };
...
(function() {
  ...
  me = function() {
    return $('div.pic_picker');
  };

  RP.pic_picker.load = function(callback) {
    var dlog, url;
    dlog = me();
    url = $(dlog).data("url");
    return $(dlog).load(url, function(responseText, textStatus, XMLHttpRequest) {
      if (jQuery.type(callback) === 'function') {
        return callback(dlog);
      }
    });
  };

  RP.pic_picker.open = function(ttl) {
    var dlog;
    if ($('div.pic_picker > *').length > 0) {
      ...
      $('a.image_preview_button').click(function() {
        return previewImg('input.icon_picker', 'div.preview img', '');
      });
      return true;
    } else {
      return false;
...
(function() {
  ...
  RP.rcp_list.onload = function() {
    $('.edit_recipe_link').bind('ajax:beforeSend', RP.edit_recipe.go);
    return $(".popup").click(RP.servePopup);
  };

  RP.rcp_list.update = function(data) {
    var replacement, replacements, _i, _len;
    if (data.action === "remove" || data.action === "destroy") {
      $('.' + data.list_element_class).remove();
      return $('.' + data.grid_element_class).remove();
      ...
      checkForLoading(".stuffypic");
      return RP.rcp_list.onload();
    }
  };

  RP.rcp_list.boostInRecent = function(list_element_class, list_element_body, targettab) {
    var tabid;
    tabid = $("#rcpquery_tabset").tabs("option", "active");
    if (tabid === targettab) {
      $("#rcplist_mine_body ." + list_element_class).remove();
      return $("#rcplist_mine_body").prepend(list_element_body);
    }
  };

  RP.rcp_list.touch_recipe = function(id) {
    if (id && (id > 0)) {
      return jQuery.get("/recipes/" + id + "/touch", {}, function(body, status, instance) {
        if (status === "success") {
          return $("." + body.touch_class).replaceWith(body.touch_body);
        }
      }, "json");
    }
  };
...
(function() {
  ...
  me = function() {
    return $('div.share_recipe.dialog');
  };

  RP.share_recipe.onload = function(dlog) {
    dlog = me();
    if ($('.share_recipe > *').length > 0) {
      return RP.tagger.onload("div.user_invitee_tokens input");
    }
  };
  ...
  jQuery(function() {
    var dlog;
    if (dlog = me()[0]) {
      return RP.share_recipe.onload(dlog);
    }
  });
...
(function() {
  ...
  RP.tagger.init = function(selector, data) {
    var prop, value, _results;
    _results = [];
    for (prop in data) {
      value = data[prop];
      _results.push($(selector).data(prop, value));
    }
    return _results;
  };

  RP.tagger.onload = function(selector) {
    if (selector == null) {
      selector = ".tagging_field";
    }
    return $(selector).each(function() {
      ...
      return $(this).tokenInput(request, {
        crossDomain: false,
