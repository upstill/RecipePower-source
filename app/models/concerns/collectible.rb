module Collectible
  ...
  included do
    ...
    attr_accessor :collectible_user_id, :collectible_comment, :collectible_private # For editng purposes: the user_pointer for the current user (collection required)
    attr_accessible :collectible_user_id, :collectible_comment, :collectible_private
  end
  
  def add_to_collection uid
    ref = user_pointer(uid, true)
  ...
  def remove_from_collection uid
    if (ref = user_pointer(uid, false)) and ref.in_collection
  ...
  def collected_by? uid
    (ref = user_pointer(uid, false)) && ref.in_collection
  end

  # Present the time-since-touched in a text format
  def touch_date uid
    if (ref = user_pointer uid, false)
      ref.updated_at
    end
  end
  ...
  def comment uid=nil
    uid ?
      (((ref = user_pointer uid, false) && ref.comment) || "") :
      @comment
  end
  ...
  def prep_params uid
    ...
    if ref = user_pointer(uid, false) # Pre-existing collection object
  ...
  def accept_params
    ...
    if collectible_user_id # It must have been set
      ...
      if ref = user_pointer(collectible_user_id, false)
  ...
  def user_pointer uid, force=true
    unless @current_ref && (@current_ref.user_id == uid)
      # A user is specified, but the currently-cached ref doesn't match
      @current_ref = force ?
        user_pointers.find_or_initialize_by(user_id: uid, entity_type: self.class.to_s, entity_id: self.id) :
