...
class LinkRef < ActiveRecord::Base
    ...
    def perform
      if ref = Reference.seek_on_url(link.uri)
        logger.debug "Link #{link.id.to_s}(#{link.uri}) already associated with tag."
      end
      if !Reference.assert(link.uri, tag)
        logger.debug "Couldn't reference tag ##{tag.id.to_s}(#{tag.name}) because "+(referents.empty? ? "it has no referents" : "Who knows why? (It has referents)")
      end
    end
