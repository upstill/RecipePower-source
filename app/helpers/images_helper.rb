module ImagesHelper
  ...
  def image_with_error_recovery url_or_object, opts_in={}
    ...
    if url_or_object.is_a? String
      ...
      fallback_img =
          if fallback_img == true && url_or_object.respond_to?(:fallback_imgdata)
            image_path(url_or_object.fallback_imgdata)
          else
            nil
          end
    end
    ...
    if options.delete :explain
      options[:data] = {
          emptyurlfallback: (fallback_img || image_path('NoPictureOnFile.png')),
    ...
    image_tag (url.if_present || fallback_img.if_present || ''), options
    # end
  end
