class CollectibleServices
  ...
  # If a new  record needs to be created, we also do QA on the provided URL
  # and dig around for a title, description, etc.
  # Either way, we also make sure that the recipe is associated with the given user
  #    and is set up to derive attributes that weren't explicitly provided
  def self.find_or_build params_or_page_ref, extractions = nil, klass=Recipe
    ...
    if params[:id]
    ...
    elsif !(entity = klass.find_by_url_and(params))  # Pre-existing => ignore extractions
      ...
      if uri.blank?
        ...
        entity = (klass==Site) ? SiteServices.find_or_build(page_ref.try(:url) || url) : klass.new(url: url)
        if uri.to_s.match %r{^#{rp_url}} # Check we're not trying to link to a RecipePower page
          ...
          entity.decorate.findings = findings # Now set the title, description, etc. as derived
        end
      end
    end
    entity
