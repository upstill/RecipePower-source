  ...
  def self.content_finder_for site_or_root, selector
    ...
    when String
      ...
      when 0 # No match found
        Rails.logger.debug "Can't find a site to match #{site_or_root}"
      when 1
        site = matching_sites.first
      else
        Rails.logger.debug "More than one site matches #{site_or_root}"
      end
    when Site
      site = site_or_root
    else
      Rails.logger.debug "FinderServices.content_finder_for must take a site or a string that matches ONE site's root"
    end
    if site
      # Create
