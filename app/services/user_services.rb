...
=end
    ...
    changed = User.all.to_a.keep_if { |u|
      ...
      if u.first_name.blank?
        if u.fullname.present?
          Rails.logger.debug "#{u.id}(#{u.email}=#{u.username}): #{u.first_name} | #{u.last_name} | #{u.fullname}"
          keep = UserServices.new(u).glean_names
          Rails.logger.debug " => #{u.first_name} | #{u.last_name}"
        else
          Rails.logger.debug "Dud: #{u.id}(#{u.email}=#{u.username}): #{u.first_name} | #{u.last_name} | #{u.fullname}"
        end
      end
      keep
    }
    changed.each { |u|
      Rails.logger.debug "'#{u.fullname}' => #{u.first_name} | #{u.last_name} (#{u.id}/#{u.email}/#{u.username})"
    }
    nil
  end
  ...
  def self.fix_names
    ...
    User.where(first_name: nil).each { |u|
      Rails.logger.debug "#{u.id}(#{u.email}): #{u.first_name} | #{u.last_name} | #{u.fullname}"
      name = gets
