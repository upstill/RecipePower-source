class NotificationsController < ApplicationController
  ...
  def accept
    if token = params[:notification_token] # defer_notification 
      ...
      if current_user && (current_user.id != @notification.target_id)
      ...
      when :share_recipe
        ...
        if current_user # Current user matches the notification: just collect the recipe
          @notification.accept
          redirect_to collect_recipe_path(@notification.shared, :uid => @notification.target_id)
