...
class FilteredPresenter
  ...
  def initialize view_context, response_service, params_hash, decorator=nil
    ...
    if subtypes.present?
      init_stream (@results_cache = ResultsCache.retrieve_or_build( (response_service.uuid || User.current_or_guest.id).to_s, subtypes, User.current_or_guest, params_hash).first)
    else
