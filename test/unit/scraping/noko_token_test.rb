...
class NokoTokenTest < ActiveSupport::TestCase
  ...
  test 'simple node enclosure' do
    ...
    newtree = assemble_tree_from_nodes ted, ted, :tag => :div, :rp_elmt_class => :rp_ingredient_tag, reattach: true
  ...
  test 'build a simple node from a single text element' do
    ...
    initialize_doc_from_file 'test/unit/scraping/noko_token_test_data.1.html'
    newtree = @nokotokens.enclose_tokens 5, 9, :tag => :div, :rp_elmt_class => :rp_ingredient_tag
    assert_equal 1, @nkdoc.css('div.rp_ingredient_tag').count
    ...
    initialize_doc_from_file 'test/unit/scraping/noko_token_test_data.1.html'
    newtree = @nokotokens.enclose_tokens 7, 9, :tag => :div, :rp_elmt_class => :rp_ingredient_tag
    assert_equal 1, @nkdoc.css('div.rp_ingredient_tag').count
    @nkdoc.css('div.rp_ingredient_tag').first == newtree
    ...
    # Enclose a substring
    initialize_doc_from_file 'test/unit/scraping/noko_token_test_data.1.html'
    newtree = @nokotokens.enclose_tokens 6, 8, :tag => :div, :rp_elmt_class => :rp_ingredient_tag
    assert_equal 1, @nkdoc.css('div.rp_ingredient_tag').count
    @nkdoc.css('div.rp_ingredient_tag').first == newtree
  ...
  test 'successfully tag an enclosing element from a single text element' do
    initialize_doc_from_file 'test/unit/scraping/noko_token_test_data.1.html'
    newtree = @nokotokens.enclose_tokens 3, 5, :tag => :div, :rp_elmt_class => :rp_ingredient_tag
    assert_equal 1, @nkdoc.css('div.rp_ingredient_tag').count
    @nkdoc.css('div.rp_ingredient_tag').first == newtree
  ...
  test 'add tokens to a tree embedded within the selection' do
    ...
    newtree = @nokotokens.enclose_tokens 4, 15, :tag => :div, :rp_elmt_class => :rp_ingredient_tag
    assert_equal 1, @nkdoc.css('div.rp_ingredient_tag').count
    assert_equal newtree, @nkdoc.css('div.rp_ingredient_tag').first
    assert_equal '0 text 1 text 2 text 3 text 4 text 5', @nkdoc.css('div.rp_ingredient_tag').inner_text.split.join(' ')
  end

  test 'move tokens outside a selected tree' do
    initialize_doc_from_file 'test/unit/scraping/noko_token_test_data.2.html'
    newtree = @nokotokens.enclose_tokens 6, 17, :tag => :div, :rp_elmt_class => :rp_ingline
    assert_equal 1, @nkdoc.css('div.rp_ingline').count
    assert_equal newtree, @nkdoc.css('div.rp_ingline').first
    assert_equal 'text 1 text 2 text 3 text 4 text 5 and more', @nkdoc.css('div.rp_ingline').inner_text.split.join(' ')
  end
  ...
  test 'appropriately enclosing a text element' do
    ...
    assert_equal 1, @nkdoc.css('div.rp_ingline').count
    assert_equal newtree, @nkdoc.css('div.rp_ingline').first
    assert_equal '3 text', @nkdoc.css('div.rp_ingline').inner_text.split.join(' ')
  end
end
