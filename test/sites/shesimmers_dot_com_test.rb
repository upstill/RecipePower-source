...
class ShesimmersDotComTest < ActiveSupport::TestCase
  include PTInterface

  # Set up the parser, trimmers, selectors for the woks_of_life site
  def setup
    @ingredients = %w{ coconut\ milk salmon\ fillet fish\ sauce kaffir\ lime\ leaves limes bird's\ eye\ chilies } # All ingredients found on the page
    @units = %w{ cup tablespoons } # All units
    @conditions = %w{  } # All conditions
    # Grammar mods, css_selector and trimmers that apply to recipes
		@grammar_mods = {
      :gm_inglist => {
        :flavor => :unordered_list,
        :list_selector => 'div.ERSIngredients ul',
...
=end
    ...
    assert_equal @sample_title, recipe.title
  end
