...
class CreateActivityNotificationTables < ActiveRecord::Migration
  # Create tables
  def change
    unless ActiveRecord::Base.connection.table_exists?('activ_notifications') do
      create_table :activ_notifications do |t|
      ...
      create_table :activ_subscriptions do |t|
        t.belongs_to :target, polymorphic: true, index: true, null: false
      ...
      add_index :activ_subscriptions, [:target_type, :target_id, :key], unique: true
    end
    end
  end
end
